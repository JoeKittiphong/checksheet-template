...
const EDWFinalRecordCutting = (props) => {
    const { prefix = "", standards = {}, placeholders = {} } = props;
    ...
    // Default Standards Logic (merged)
    ...
    // If dataRows passed, use them. Else default.
    const effectiveRecordData = props.dataRows || [
        {
            id: "C0001",
    ...
    const recordColumns = [
        { header: "", key: "id", width: "15%", className: "bg-gray-300 font-bold text-[9px]" },
        {
            ...
            className: "text-[9px]",
            render: (val, row) => {
                const name = row.voltage_key || (row.isFooter ? `${prefix}wire_broken` : `${prefix}${row.id.toLowerCase()}_v`);
                ...
                const disabled = row.disabled?.voltage;

                return (
                    <input
                        {...register(name, { required: !row.isFooter && !disabled })}
                        disabled={disabled}
                        placeholder={row.placeholderV}
                        className={`w-full h-full text-center outline-none ${isError ? 'bg-red-500 placeholder:text-white' : (validationClass ? validationClass : 'bg-transparent')} placeholder:text-gray-300 text-[7px] ${disabled ? 'bg-gray-200' : ''}`}
                    />
                );
            }
        },
        {
            ...
            render: (val, row) => {
                ...
                const name = row.current_key || `${prefix}${row.id.toLowerCase()}_a`;
                const currentVal = allValues?.[name];
                const range = parseRange(std[row.stdKey]?.a);
                const isError = errors[name];
                const validationClass = getUIVallidationClass(currentVal, range);
                const disabled = row.disabled?.current;
        ...
        },
        {
            ...
            className: "text-[9px]",
            render: (val, row) => {
                const name = row.speed_key || (row.isFooter ? `${prefix}time_s` : `${prefix}${row.id.toLowerCase()}_speed`);
                const currentVal = allValues?.[name];
                const range = parseRange(std[row.stdKey]?.speed);
                const isError = errors[name];
                const validationClass = getUIVallidationClass(currentVal, range);
                const disabled = row.disabled?.speed;
        ...
        },
        {
            ...
            className: "text-[9px]",
            render: (val, row) => {
                const label = row.timeLabel || row.time_label || "";
                const name = row.isFooter ?
                    (row.key || `${prefix}h081_time`) :
                    (row.time_key || `${prefix}${row.id.toLowerCase()}_time`);
                ...
                const disabled = row.disabled?.time;

                return (
                    <div className={`flex items-center px-1 h-full gap-1 ${isError ? 'bg-red-500' : validationClass} text-[9px] ${disabled ? 'bg-gray-200' : ''}`}>
            ...
            <div className="flex gap-2 font-sans text-[8px] w-full items-start">
                <div className="w-[55%]">
                    <FormQuickTable
                        ...
                        data={effectiveRecordData}
                    />
                </div>
                <div className="w-[45%]">
                    <FormQuickTable
                        columns={stdColumns}
                        data={effectiveStdData}
