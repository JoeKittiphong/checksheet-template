...
// Images
import imgMachineTop from '@/assets/FAMB0007_V3/FAMB0007-28-1.PNG';
import imgMachineBottom from '@/assets/FAMB0007_V3/FAMB0007-28-2.PNG';

function Page16() {
    const { setValue, control, register, formState: { errors } } = useFormContext();
    const isNA = useWatch({ name: 'p16_28_na', control }) === 'N/A';
    ...
    useEffect(() => {
        ...
        register('p16_28_points_top', validationRule);
        register('p16_28_points_bottom', validationRule);
    }, [register, isNA]);
    ...
    const pointsBottom = useMemo(() => [
        { id: 9, x: 25, y: 15, label: "LM Guide W axis\n- สกรูล็อคแน่นทุกตัว\n- มาร์คสีสกรูทุกตัว\n- ทาน้ำมันสนบริเวณรอบ ๆ \nเรียบร้อย", checked: null, textPosition: "right" },
        { id: 10, x: 15, y: 40, label: "W axis (Manual)\n- สกรูล็อคแน่นทุกตัว\n- มาร์คสีสกรูทุกตัว\n- Move ขึ้นลงได้ปกติ \nไม่ติดขัด", checked: null, textPosition: "right" },
        { id: 11, x: 10, y: 60, label: "Motor X axis\n- สกรูล็อคแน่นทุกตัว\n- มาร์คสีสกรูทุกตัว", checked: null, textPosition: "right" },
    ...
    // Automated N/A logic for Section 28
    useEffect(() => {
        if (isNA) {
            const syncNA = (points) => points.map(p => ({ ...p, checked: 'N/A' }));
            setValue('p16_28_points_top', syncNA(pointsTop));
            setValue('p16_28_points_bottom', syncNA(pointsBottom));
        }
    ...
    return (
        <A4Paper content={content} currentPage={16}>
            <div className="h-full relative flex flex-col text-[13px]">
                ...
                {/* 28. Double check machine (W axis (Manual)) */}
                <section className="flex-1 flex flex-col pt-2">
                    <div className="flex gap-2 items-center mb-1">
                        <p className="font-bold text-base">28. Double check machine (W axis (Manual))</p>
                        <FormItemCheck name="p16_28_na" label="N/A" />
                    </div>
                    ...
                    <div className="mt-8 flex justify-end">
                        <div className="flex gap-10">
                            <div className="flex flex-col items-center">
                                <FormCheckedBox label="(Operator)/Date" name="p16_28_check_by_date_op" />
                            </div>
                            <div className="flex flex-col items-center">
                                <FormCheckedBox label="(Leader up)/Date" name="p16_28_check_by_date_leader" />
                            </div>
                        </div>
                    </div>
                </section>
