...
โครงสร้างของระบบแบ่งออกเป็น 2 ส่วนหลัก คือ Frontend (Checksheet Template) และ Backend (Server & Admin):

### 7.1 Root Directory: `template-E-checksheet`
โฟลเดอร์รวบรวมโปรเจคทั้งหมด ประกอบด้วย:
- **checksheet-template**: โปรเจค Frontend หลัก
- **server-checksheet**: โปรเจค Backend และ Admin Panel
- **nginx**: การตั้งค่า Web Server (ถ้ามี)
- **build all.bat**: สคริปต์สำหรับ Build โปรเจคทั้งหมด

### 7.2 Frontend: `checksheet-template`
สร้างด้วย **React + Vite + TailwindCSS** โดยใช้ `react-hook-form` ในการจัดการข้อมูล
- **จุดประสงค์**: ใช้สำหรับสร้างหน้าฟอร์ม E-Checksheet ต่างๆ ที่พนักงานจะใช้งาน
- **โครงสร้างสำคัญ**:
  - `src/checksheet/`: เก็บ Source Code ของแต่ละ Checksheet (แบ่งเป็นโฟลเดอร์ตามชื่อ Checksheet)
  - `src/components/`: Component กลางที่ใช้ร่วมกัน เช่น `FormQuickTable`, `VirtulKeypad`
  - `src/utils/`: ฟังก์ชันกลาง เช่น Validation (`validationUtils.js`), การคำนวณ
  - `vite.config.js`: การตั้งค่า Build รวมถึง PWA (Offline Support)

### 7.3 Backend: `server-checksheet`
สร้างด้วย **Node.js (Express)** ทำหน้าที่เป็น API Server และ Web Server
- **จุดประสงค์**: จัดการ Authentication, เชื่อมต่อ Database (PostgreSQL), และ Serve ไฟล์ Static ของหน้าฟอร์ม
- **โครงสร้างสำคัญ**:
  - `as_server.js`: **Entry Point** ของระบบ ทำหน้าที่รัน Server และกำหนด Route
  - `routes/`: API Routes ต่างๆ (Auth, Users, Forms, DB)
  - `checksheet_admin/`: **Sub-project** (Vite App) สำหรับหน้า Admin Panel ที่ใช้จัดการ Users และดู Report
  - `checksheet_form/`: โฟลเดอร์ที่เก็บไฟล์ HTML/JS ของฟอร์มที่ Build มาแล้ว (จาก `checksheet-template`)

### 7.4 การทำงานร่วมกัน (Workflow)
1. **Develop**: สร้างฟอร์มใน `checksheet-template`
2. **Build**: สั่ง Build ฟอร์มจะได้ไฟล์ Static ไปลงที่ `server-checksheet/checksheet_form`
3. **Serve**: เมื่อรัน `as_server.js` ระบบจะ Serve:
   - หน้า Admin Panel ที่ Root Path (`/`) หรือ `/admin`
   - หน้าฟอร์มต่างๆ ที่ path `/form/<Checksheet_Name>`
4. **Data**: ข้อมูลจากฟอร์มจะถูกส่งผ่าน API ไปบันทึกที่ Database ผ่าน Express Server
